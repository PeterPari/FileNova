...
export const TabBar = ({
    ...
    return (
        <div className="bg-surface border-b border-base flex items-center gap-1 px-2 py-1 overflow-x-auto">
            {tabs.map((tab) => (
                <div
                    key={tab.id}
                    className={`group relative flex items-center gap-2 px-3 py-1.5 rounded-t min-w-[120px] max-w-[200px] cursor-pointer ${
                        tab.id === activeTabId
                            ? 'bg-base border-t-2 border-blue-500'
                ...
                >
                    ...
                    {tab.isPinned && <Pin size={12} className="text-muted flex-shrink-0" />}
                    ...
                    {showMenu === tab.id && (
                        <div className="absolute top-full left-0 mt-1 bg-base border border-base rounded shadow-lg z-10 py-1 min-w-[150px]">
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                                    onTabPin(tab.id);
                            ...
                            {!tab.isPinned && (
                                <button
                                    ...
                                    className="w-full px-3 py-1.5 text-left text-sm hover:bg-surface-hover flex items-center gap-2 text-red-500"
                                >
                                    <X size={14} />
                                    Close Tab
                                </button>
    ...
    return (
        <>
            <div className="relative">
                ...
                {showDropdown && (
                    <div className="absolute top-full left-0 mt-1 bg-base border border-base rounded shadow-lg z-10 py-1 min-w-[200px]">
                        <button
                            onClick={() => {
                                setShowModal(true);
                                setShowDropdown(false);
                        ...
                        {workspaces.length > 0 && (
                            <>
                                ...
                                <div className="px-3 py-1 text-xs text-muted uppercase font-semibold">
                                    Load Workspace
                                </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="bg-base rounded-lg shadow-xl p-6 w-full max-w-md">
                        ...
                        <input
                            ...
                            className="w-full px-3 py-2 border border-base rounded bg-transparent mb-4"
                            autoFocus
                        />
                        <div className="flex gap-2 justify-end">
                            <button
                                ...
                                className="px-4 py-2 text-sm bg-surface-hover hover:bg-surface-active rounded"
                            >
                                Cancel
                            </button>
                            <button
