...
const SuggestionEditor: React.FC<SuggestionEditorProps> = ({ initialPlan, onSave, onCancel }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-8 z-50">
            <div className="bg-base rounded-xl w-full max-w-4xl max-h-full flex flex-col border border-base" style={{ boxShadow: 'var(--shadow-xl)' }}>
                <div className="p-6 border-b border-base flex justify-between items-center">
                    ...
                    <button onClick={onCancel} className="text-muted hover:text-primary">✕</button>
                </div>

                <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        <div className="bg-surface rounded-lg border border-base p-4">
                            <div className="text-xs text-muted uppercase tracking-wider mb-3">Files</div>
                            <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar">
                                {moves.map((move, idx) => (
                                    <div
                                        ...
                                        className="bg-base border border-base rounded px-3 py-2 text-sm text-disabled cursor-move"
                                ...
                                {moves.length === 0 && (
                                    <div className="text-muted text-sm">No files in this plan.</div>
                                )}
                            </div>
                        </div>

                        <div className="bg-surface rounded-lg border border-base p-4">
                            <div className="text-xs text-muted uppercase tracking-wider mb-3">Destinations</div>
                            <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar">
                                {destinationFolders.map((folder) => (
                                    <div
                                        ...
                                        className="bg-base border border-dashed border-base rounded px-3 py-2 text-xs text-disabled"
                                        title={folder}
                                    >
                                        {folder}
                                    </div>
                                ...
                                {destinationFolders.length === 0 && (
                                    <div className="text-muted text-sm">No destination folders yet.</div>
                                )}
                            </div>
                    ...
                    <div className="space-y-4">
                        {moves.map((move, idx) => (
                            <div key={idx} className="flex gap-4 items-start bg-surface p-3 rounded border border-base">
                                <button
                                ...
                                <div className="flex-1 grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <div className="text-sm text-disabled truncate" title={move.file_path}>
                                            {move.file_path}
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-base border border-base rounded px-2 py-1 text-sm text-green-400 focus:outline-none focus:border-focus"
                                        />
                                    </div>
                                </div>
                            </div>
                ...
                <div className="p-6 border-t border-base bg-base flex justify-end gap-3 rounded-b-xl">
                    <button
                        onClick={onCancel}
                    ...
                    <button
                        onClick={handleSave}
                        className="px-6 py-2 bg-accent-primary hover:bg-blue-500 text-white rounded font-bold"
                    >
                        Save & Update Plan
                    </button>
                </div>
